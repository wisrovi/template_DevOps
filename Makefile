...
app-lint:
	docker-compose run --rm sidecar pylint ../app --evaluation="max(0, 0 if fatal else 10.0 - ((float(5 * error + warning + refactor + convention) / statement) * 10))" --msg-template="{path}:{line}: [{msg_id}({symbol}), {obj}] {msg}" > src/config_DevOps/.pylint_cache/pylint-report.txt --reports=n --confidence=HIGH,CONTROL_FLOW,INFERENCE,INFERENCE_FAILURE,UNDEFINED --persistent=true --output-format=json:report_DevOps/pylint.json,text:report_DevOps/pylint.txt,colorized
